...
def guardar_guia(store_name: str, brand: str, url: str, sender_name: str) -> bool:
    ...
    try:
        ...
        return supabase_client.upsert('guide_logs', data, on_conflict="url") 
...
def mostrar_analisis_historico_kpis():
    ...
    col1, col2, col3_ = st.columns(3)
    with col1:
        if st.button("💾 Exportar a Excel", use_container_width=True):
            # Lógica de exportación
            pass # Lógica omitida para concisión, pero se mantiene la estructura
    ...
    fecha_max_date = df['fecha'].max().date()
    fecha_min_date = df['fecha'].min().date()

    periodos_sigma = {
        "Último Mes": (fecha_max_date - timedelta(days=30), fecha_max_date),
        "Último Trimestre": (fecha_max_date - timedelta(days=90), fecha_max_date),
        "Todo el Historial": (fecha_min_date, fecha_max_date)
    }
...
def mostrar_gestion_trabajadores_kpis():
    ...
    try:
        ...
        response = supabase_client.client.from_('trabajadores').select('*').order('equipo,nombre', desc=False).execute()
        ...
        with st.form("form_nuevo_trabajador"):
            ...
            if submitted:
                if nuevo_nombre:
                    try:
                        response = supabase_client.client.from_('trabajadores').select('*').eq('nombre', nuevo_nombre).execute()
                        if response and hasattr(response, 'data') and response.data:
                            st.markdown("<div class='error-box animate-fade-in'>❌ El trabajador ya existe.</div>", unsafe_allow_html=True)
                            st.session_state.show_preview = False
                        else:
                            supabase_client.client.from_('trabajadores').insert({
                                'nombre': nuevo_nombre, 
                                'equipo': nuevo_equipo,
                                'activo': True
                            }).execute()
        ...
        if trabajadores:
            ...
            if trabajadores_activos:
                ...
                if st.button("Eliminar Trabajador", use_container_width=True):
                    try:
                        supabase_client.client.from_('trabajadores').update({'activo': False}).eq('nombre', trabajador_eliminar).execute()
...
def obtener_tienda_por_id(tienda_id: int) -> Optional[Dict]:
    ...
    try:
        response = supabase_client.client.from_('guide_stores').select('*').eq('id', tienda_id).execute()
        if response and hasattr(response, 'data') and response.data:
...
def agregar_tienda(nombre: str, direccion: str, telefono: str) -> bool:
    ...
    try:
        ...
        response = supabase_client.client.from_('guide_stores').insert(data).execute()
...
def actualizar_tienda(tienda_id: int, nombre: str, direccion: str, telefono: str) -> bool:
    ...
    try:
        ...
        response = supabase_client.client.from_('guide_stores').update(data).eq('id', tienda_id).execute()
...
def eliminar_tienda(tienda_id: int) -> bool:
    ...
    try:
        response = supabase_client.client.from_('guide_stores').delete().eq('id', tienda_id).execute()
            ...
            if st.button("🗑️ Eliminar Guías Seleccionadas", use_container_width=True):
                if guias_a_eliminar:
                    ...
                    for guia_id in guias_a_eliminar:
                        # Se asume que la función eliminar_guia existe y usa supabase_client
...
def mostrar_modulo_cubicaje():
    ...
    df_trabajadores = obtener_trabajadores()
    # Asume que Jessica Suarez, Luis Perugachi, Andrea Malquin hacen packing
...
def mostrar_modulo_kpi_adicionales():
    ...
    with tab1:
        # Se llama al módulo de precisión del picking
...
def mostrar_gestion_avanzada():
    ...
    with tab1:
        # Se llama al módulo de Cubicaje
